...
async function fetchSheetData() {
  ...
  // Helper: fetch and force UTF-8 decode (handles Bengali/non-ASCII correctly)
  ...
  const [orgT, conT, emT, banT, setT] = await Promise.all([
    fetchText(sheetURL(CONFIG.ORG_SHEET)),
    fetchText(sheetURL(CONFIG.CONTACTS_SHEET)),
    fetchText(sheetURL(CONFIG.EMERGENCY_SHEET)),
    fetchText(sheetURL(CONFIG.BANNER_SHEET)),
    fetchText(sheetURL(CONFIG.SETTINGS_SHEET)),
  ]);

  // Banners: accept rows where is_active is TRUE or not specified
  ...
  // Settings: key-value rows -> plain object
  const settingsObj = {};
...
async function fetchSettingsOnly() {
  ...
  try {
    ...
    // Force UTF-8 decode so Bengali/non-ASCII hint text shows correctly
    const buf  = await res.arrayBuffer();
    const text = new TextDecoder('utf-8').decode(buf);
    const obj  = {};
    parseCSVRaw(text).forEach(row => {
