...
// Used for tel:, wa.me links so calls work on any phone
...
function makeExtCallLink(contact) {
  ...
  const cleanExt  = toEngDigits(contact.ext).replace(/\D/g, '');
  ...
  list.innerHTML = contacts.map(c => {
    ...
    return `<div class="contact-card" onclick="navigate('contact-detail',{contactId:'${c.contact_id}'})" role="button" tabindex="0">
      ...
      <div class="contact-card-actions">
        ${c.phone?`<a href="tel:${normPhone(c.phone)}" class="action-btn call-btn" onclick="event.stopPropagation()" title="Call Direct">${ICON_CALL}</a>`:''}
...
function renderContactDetail(contactId) {
  ...
  if (c.phone)      items.push({icon:'📞',label:'Phone',        value:c.phone,  href:`tel:${normPhone(c.phone)}`});
