...
function saveSession() {
  const hours = CONFIG.SESSION_HOURS || 8;
  localStorage.setItem(CONFIG.STORAGE.SESSION,
    JSON.stringify({ expiry: Date.now() + hours * 3600 * 1000 }));
}
...
function attemptUnlock() {
  ...
  if (entered === correctPw) {
    _failCount = 0;
    hide($('lock-hint-box'));
    if (remember && remember.checked) saveSession();
    hideLockAndStart();
  } else {
    _failCount++;
...
// ── Extension Call Link ──────────────────────────────────────────
// Builds: tel:+88029669974,,101
// Mobile dials main number → waits (,,) → dials extension automatically
function makeExtCallLink(contact) {
  if (!contact.ext) return null;
  const org = getOrgById(contact.org_id);
  if (!org || !org.phone) return null;
  // Normalize: keep leading +, strip everything else except digits
  const raw = org.phone.trim();
  const hasPlus = raw.startsWith('+');
  const digits = raw.replace(/\D/g, '');
  const cleanMain = (hasPlus ? '+' : '') + digits;
...
function renderOrgPage(orgId) {
  ...
  if (org.phone)   items.push({icon:'📞',label:'Phone',  value:org.phone,  href:`tel:${org.phone}`});
  ...
  list.innerHTML = contacts.map(c => {
    ...
    return `<div class="contact-card" onclick="navigate('contact-detail',{contactId:'${c.contact_id}'})" role="button" tabindex="0">
      ...
      <div class="contact-card-actions">
        ${c.phone?`<a href="tel:${c.phone}" class="action-btn call-btn" onclick="event.stopPropagation()" title="Call Direct">${ICON_CALL}</a>`:''}
        ...
        ${hasWA&&c.phone?`<a href="https://wa.me/${c.phone.replace(/\D/g,'')}" class="action-btn wa-btn" onclick="event.stopPropagation()" target="_blank" rel="noopener" title="WhatsApp">${ICON_WA}</a>`:''}
...
function renderEmergencyPage() {
  ...
  if (natGrid) {
    natGrid.innerHTML = NATIONAL_EMERGENCY.map(n => `
      <a href="tel:${n.number}" class="nat-card" style="--nc:${n.color}">
  ...
  if (list) list.innerHTML = items.map(e => {
    ...
    return `<div class="emergency-contact-card">
      ...
      <a href="tel:${e.phone}" class="emergency-call-btn" title="Call ${e.name}">
...
function renderContactDetail(contactId) {
  ...
  if (c.phone)      items.push({icon:'📞',label:'Phone',        value:c.phone,  href:`tel:${c.phone}`});
  // Extension: shows org main number + ext, with call link
  if (c.ext && org && org.phone) {
    items.push({
      icon: '🔢',
      label: 'Ext.',
      value: c.ext,
      sub: `Dial via ${org.org_name}: ${org.phone} → Ext. ${c.ext}`,
      href: extLink,
    });
  }
  ...
  const btns = [];
  if (c.phone)        btns.push(`<a href="tel:${c.phone}" class="action-large call-large">${ICON_CALL}<span>Call Direct</span></a>`);
  if (extLink)        btns.push(`<a href="${extLink}" class="action-large ext-large"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M9 9h.01M12 9h.01M15 9h.01M9 12h.01M12 12h.01M15 12h.01M9 15h.01M12 15h.01"/></svg><span>Call Ext. ${c.ext}</span></a>`);
  if (hasWA&&c.phone) btns.push(`<a href="https://wa.me/${c.phone.replace(/\D/g,'')}" target="_blank" rel="noopener" class="action-large wa-large">${ICON_WA}<span>WhatsApp</span></a>`);
...
async function initApp() {
  // 1. Valid session? Skip lock screen entirely.
  if (checkSession()) {
    hide($('lock-screen'));
    await startApp();
    return;
  ...
  // 2. No session — fetch Settings from Sheet (for password + hint).
  //    Do this BEFORE showing the lock screen so auth uses the latest values.
  ...
  try {
    ...
    if (fresh) {
      ...
      State.settings = loadCachedSettings() || SAMPLE_SETTINGS;
    }
  } catch {
    State.settings = loadCachedSettings() || SAMPLE_SETTINGS;
  }
  ...
  // 3. Show lock screen and wait for correct password.
  setupLockListeners();
}
